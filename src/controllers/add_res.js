...
module.exports = {
    ...
    post: (req, res) => {
        ...
        if (!foundRes) {
            ...
            fs.writeFileSync(path.join(process.cwd(), 'src', 'model', 'restaurants.json'), JSON.stringify(restaurants, null, 4))
    ...
    delete_res: (req, res) => {
        ...
        if (all_res) {
            fs.writeFileSync(path.join(process.cwd(), 'src', 'model', 'restaurants.json'), JSON.stringify(all_res, null, 4))
            res.redirect('/dashboard')
        } else {
            res.redirect('/login')
        }
